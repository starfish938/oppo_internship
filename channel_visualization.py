...
for snr in snr_range:
    ...
    nmse_vals = [ofdm_simulation_with_ls(snr, channel_h)[1] for _ in range(100)]

    nmse_list.append(np.mean(nmse_vals))


plt.figure(figsize=(8,6))

plt.plot(snr_range, nmse_list, 'r-o', linewidth=2)

plt.xlabel("SNR (dB)")
plt.ylabel("NMSE (dB)")

plt.title("LS信道估计 NMSE vs SNR")
...
def visualize_channel_estimation(channel_h_time, snr_db=20):
    ...
    # 真实信道
    H_true = np.fft.fft(channel_h_time, N_fft)


    # ===== 画图 =====
    plt.figure(figsize=(10,6))

    plt.plot(np.abs(H_true), 'k', linewidth=2, label="True Channel")

    plt.plot(np.abs(H_ls_all), 'r--', linewidth=2, label="LS Estimated Channel")

    plt.scatter(pilot_indices, np.abs(H_ls_pilots), color='blue', label="Pilot Estimates")

    plt.xlabel("Subcarrier Index")
    plt.ylabel("Channel Magnitude")

    plt.title("Channel Estimation Visualization")
...
visualize_channel_estimation(channel_h, snr_db=20)